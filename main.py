...
def get_images_info_and_annotations(opt):
    path = Path(opt.path_images)
    path_annotations = Path(opt.path_annotations)
    annotations = []
    images_annotations = []
    if path.is_dir():
    ...
    for file_path in file_paths:
        ...
        if opt.yolo_subdir:
            ...
            annotations_path = path_annotations / label_file_name
...
def main(opt):
    ...
    if opt.debug is True:
        ...
        # Crear nuevo JSON insertando al inicio
        new_data = {}
